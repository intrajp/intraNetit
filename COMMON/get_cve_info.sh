...
VALUE_VERSION2=$(echo "${VALUE_VERSION2}" | sed -e 's/.*warning">//' | sed -e 's/<.*>//' | sed -e 's///g')
SCORE_VERSION2=$(echo "${VALUE_VERSION2}" | awk -F" " '{ print $1 }' | sed -e 's///g')
SEVERITY_VERSION2=$(echo "${VALUE_VERSION2}" | awk -F" " '{ print $2 }' | sed -e 's///g')
